                ...
                <div class="tab-pane fade" id="pemilik" role="tabpanel">
                    ...
                    <?php else: ?>
                        <table class="table table-bordered">
                            ...
                            <tr><th>Direktur</th><td><?= htmlspecialchars($pengurus['direktur']) ?></td></tr>
                    ...
                    <?php else: ?>
                        <div class="table-responsive">
                            <table class="table table-bordered table-striped align-middle">
                                ...
                                <tbody>
                                    <?php foreach ($tenaga_list as $idx => $t): ?>
                                        <?php
                                        $t_verified_by = $t['verified_by'] ?? null;
                                        $t_verified_at = $t['verified_at'] ?? null;
                                        if ($t_verified_by && $t_verified_at) {
                                            $t_status_text = 'TERVERIFIKASI';
                                            $t_badge_class = 'success';
                                        } else {
                                            $t_status_text = 'BELUM';
                                            $t_badge_class = 'secondary';
                                        }
                                        ?>
                                        <tr>
                                            <td><?= $idx + 1 ?></td>
                                            ...
                                            <td>
                                                <span class="badge bg-<?= $t_badge_class ?>"><?= $t_status_text ?></span>
                                            </td>
                                        </tr>
                                    <?php endforeach; ?>
                                </tbody>
                        ...
                        // Agregat status
                        if ($tenaga_ok) {
                            $tenaga_status_text = "SEMUA TENAGA AHLI TERVERIFIKASI ({$tenaga_verified_count}/{$tenaga_count})";
                            $tenaga_badge_class = 'success';
                        } elseif ($tenaga_verified_count > 0) {
                            $tenaga_status_text = "SEBAGIAN TENAGA AHLI TERVERIFIKASI ({$tenaga_verified_count}/{$tenaga_count})";
                            $tenaga_badge_class = 'warning';
                        } else {
                            $tenaga_status_text = "BELUM ADA TENAGA AHLI TERVERIFIKASI ({$tenaga_verified_count}/{$tenaga_count})";
                            $tenaga_badge_class = 'secondary';
                        }

                        // Ambil notes dari salah satu baris (opsional)
                        $tenaga_notes_current = '';
                        foreach ($tenaga_list as $t) {
                            if (!empty($t['notes'])) {
                                $tenaga_notes_current = $t['notes'];
                                break;
                            }
                        }
                        ?>
                        ...
                        <div class="mb-3">
                            <label class="form-label"><strong>Status Verifikasi Tenaga Ahli (Agregat)</strong></label><br>
                            <span class="badge bg-<?= $tenaga_badge_class ?>">
                                <?= htmlspecialchars($tenaga_status_text) ?>
                            </span>
                        </div>

                        <div class="mb-3">
                            <label class="form-label"><strong>Catatan Admin (Saat Ini)</strong></label>
                            <textarea class="form-control" rows="3" readonly><?= htmlspecialchars($tenaga_notes_current) ?></textarea>
                        </div>
                        ...
                        <form method="POST">
                            ...
                            <div class="mb-3">
                                <label class="form-label"><strong>Ubah Status Verifikasi Tenaga Ahli (Agregat)</strong></label>
                                <select name="verif_status" class="form-select" required>
                                    <option value="0" <?= !$tenaga_ok ? 'selected' : '' ?>>Belum Diverifikasi (Reset Semua)</option>
                                    <option value="1" <?= $tenaga_ok  ? 'selected' : '' ?>>Set Semua Terverifikasi</option>
                                </select>
                            </div>

                            <div class="mb-3">
                                <label class="form-label"><strong>Catatan untuk Perusahaan (opsional)</strong></label>
                                <textarea class="form-control" name="notes" rows="4"><?= htmlspecialchars($tenaga_notes_current) ?></textarea>
                            </div>

                            <div class="d-flex justify-content-end">
                                <button type="submit" class="btn btn-success px-4">Simpan Verifikasi Tenaga Ahli</button>
                            </div>
                        </form>
                    <?php endif; ?>
                </div>
