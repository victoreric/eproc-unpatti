...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && (($_POST['tab'] ?? '') === 'tenaga_ahli')) {
    ...
    // __DIR__ = .../htdocs/eproc-unpatti/pengadaan/public
    $htdocsRoot = dirname(__DIR__, 3);
    $uploadDir  = $htdocsRoot . '/storage_secure/tenaga_ahli/';

    if (!is_dir($uploadDir)) {
        @mkdir($uploadDir, 0755, true);
    }

    // Helper upload file PDF (CV / Sertifikat)
    $uploadPdfTA = function($fieldName, $prefix) use ($uploadDir, $htdocsRoot, $user_id) {
        if (!isset($_FILES[$fieldName]) || $_FILES[$fieldName]['error'] === UPLOAD_ERR_NO_FILE) {
        ...
        $newFileName = $prefix . '_' . $user_id . '_' . time() . '.pdf';
        $destPathAbs = $uploadDir . $newFileName;
        $destPathRel = 'storage_secure/tenaga_ahli/' . $newFileName;
    ...
    */
    ...
    /*
     ---------------------------------------------------------------
      ...
      (untuk saat ini hanya tambah & hapus, edit bisa ditambahkan nanti)
     ---------------------------------------------------------------
    */
    if ($action === 'add') {
        // Sanitasi input form
        ...
        $keahlian         = isset($_POST['keahlian']) ? trim($_POST['keahlian']) : '';
        $pengalaman_tahun = isset($_POST['pengalaman_tahun']) ? (int)$_POST['pengalaman_tahun'] : null;
        ...
        $file_cv_path         = $uploadPdfTA('file_cv', 'cv_ta');
        $file_sertifikat_path = $uploadPdfTA('file_sertifikat', 'sert_ta');
        ...
        if (file_exists(__DIR__ . '/../includes/notify.php')) {
            ...
            createNotification(
                ...
                $user_id,
                "create_tenaga_ahli",
                "Perusahaan " . ($companyForTA['name'] ?? 'Perusahaan') . " menambahkan Tenaga Ahli baru.",
                $ta_id,
                "tenaga_ahli"
            );
        }
     ...
     ---------------------------------------------------------------
    */
            function uploadPdf($field, $folder, $old_file = null) {
                global $user_id;
                ...
                if ($ext !== 'pdf') {
                    return $old_file; // atau bikin alert
                }

                $baseDir = dirname(__DIR__) . "/../storage_secure/$folder/";
                if (!is_dir($baseDir)) mkdir($baseDir, 0755, true);

                $newName = "{$field}_{$user_id}_" . time() . ".pdf";
                $absPath = $baseDir . $newName;
                $relPath = "storage_secure/$folder/" . $newName;

                if (move_uploaded_file($_FILES[$field]['tmp_name'], $absPath)) {
                    // hapus file lama
                    if ($old_file && file_exists(dirname(__DIR__) . '/../' . $old_file)) {
                        @unlink(dirname(__DIR__) . '/../' . $old_file);
                    }
                    return $relPath;
                }
    ...
    // AMBIL DATA LAMA
    $stmtOld = $pdo->prepare("SELECT * FROM tenaga_ahli WHERE id = ? AND user_id = ?");
    $stmtOld->execute([$ta_id, $user_id]);
    $old = $stmtOld->fetch();
    ...
    // HANDLE FILE CV (opsional)
    $file_cv_path = $old['file_cv'];

    if (!empty($_FILES['file_cv']['name'])) {
        $file_cv_path = uploadPdf('file_cv', 'tenaga_ahli', $old['file_cv']);
    }

    // HANDLE FILE SERTIFIKAT (opsional)
    $file_sertifikat_path = $old['file_sertifikat'];

    if (!empty($_FILES['file_sertifikat']['name'])) {
        $file_sertifikat_path = uploadPdf('file_sertifikat', 'tenaga_ahli', $old['file_sertifikat']);
    }

    // UPDATE DATABASE
    $upd = $pdo->prepare("
        UPDATE tenaga_ahli SET 
            nama=?, jabatan=?, pendidikan=?, pengalaman_tahun=?, keahlian=?, 
            file_cv=?, file_sertifikat=?, updated_at=NOW()
    ...
    // jika ada action lain di masa depan, bisa ditambah di sini
}
                            ...
                            <div class="col-md-12 grid-margin stretch-card">
                                <div class="card bg-light">
                                    ...
                                    <div class="card-body">
                                            ...
                                            <form method="POST" enctype="multipart/form-data" novalidate>
                                                ...
                                                <div class="row">
                                                    ...
                                                    <div class="col-md-6">
                                                        <!-- KEAHLIAN -->
                                                        <div class="mb-3">
                                                            <label class="form-label">Keahlian Utama / Spesialisasi</label>
                                                            <textarea name="keahlian" class="form-control" rows="3" placeholder="Misal: Manajemen Proyek Konstruksi, Pengawasan Struktur Gedung, dsb."></textarea>
                                                        </div>
                                                               ...
                                                               <td>
                                                                ...
                                                                <td class="text-center d-flex flex-column gap-1">
                                                                    ...
                                                                    <button 
                                                                        ...
                                                                        onclick="editTenagaAhli(<?= htmlspecialchars(json_encode($ta)) ?>)">
                                                                        Edit
                                                                    </button>
                                        ...
                                        <div class="alert alert-info p-2 mt-3">
                                            <strong>Keterangan:</strong><br>
                                            ✏️ Data tenaga ahli bersifat dinamis, silakan tambahkan sesuai kebutuhan.<br>
                                            🔒 Jika nantinya ingin menambah kolom verifikasi (misal diverifikasi oleh Admin LPSE/Eproc), bisa menyesuaikan struktur tabel <code>tenaga_ahli</code>.
                                        </div>
                            ...
                            =========================================== -->
                                <div class="modal fade" id="modalEditTA" tabindex="-1">
                                            ...
                                            <div class="modal-body">
                                                ...
                                                <div class="row">
                                                    ...
                                                    <div class="col-md-6">
                                                        ...
                                                        <input type="number" name="pengalaman_tahun" id="edit_pengalaman" class="form-control" required>

                                                        <label class="form-label mt-3">Keahlian *</label>
                                                        <textarea name="keahlian" id="edit_keahlian" class="form-control" required></textarea>
                                                    </div>
                                                </div>
                                                ...
                                                <div>
                                                    <label class="form-label">Ganti File CV (PDF, opsional)</label>
                                                    <input type="file" name="file_cv" class="form-control">

                                                    <label class="form-label mt-3">Ganti Sertifikat (PDF, opsional)</label>
                                                    <input type="file" name="file_sertifikat" class="form-control">
                                                </div>
                                    ...
                                    function editTenagaAhli(ta) {
                                        ...
                                        let modal = new bootstrap.Modal(document.getElementById('modalEditTA'));
                                        modal.show();
                                    }
                                    </script>
                        </div>
